...
int display_intro(struct Player *player){
	...
	while (1){
		...
		switch(option){
			...
			case 2:
				stopMusic();
				pthread_create(&musicThread, NULL, music_thread, "enginestarting.mp3");
				system("cls");
...
double Show_Score(clock_t start_time) { 
    double elapsed_time = (double)(clock() - start_time) / CLOCKS_PER_SEC;
    printf("Player survived for: %.0f seconds\n", elapsed_time);
    return elapsed_time;
}

int storing_Score(double score, struct Player *player){
